...
class EntropySampling(Strategy):
    ...
    def query(self, leave_pbar=True, cur_epoch=None, gpu_collect=True):
        ...
        for i, data in enumerate(self.unlabelled_loader):
            with torch.no_grad():
                ...
                for bs in range(final_full_cls_logits.shape[0]):
                    ...
                    select_list.append([data['img_metas'].data[0][bs]['sample_idx'], data['img_metas'].data[0][bs]['scene_token'], aggregated_values.item()])
        ...
        #         if select[1] in scene_dict:
        #             scene_dict[select[1]] += select[2]
        #         else:
        #             scene_dict[select[1]] = select[2]


        #     select_scene_dict = dict(sorted(scene_dict.items(), key=lambda item: item[1]))
        #     unlabelled_scene_num = len(select_scene_dict.keys())

        #     if unlabelled_scene_num >= self.cfg.active_train.select_sequences:
        #         selected_scene_tokens = list(select_scene_dict.keys())[unlabelled_scene_num - self.cfg.active_train.select_sequences:]
        #     else:
        #         selected_scene_tokens = list(select_scene_dict.keys())


        #     for select in select_list:
        #         if select[1] in selected_scene_tokens:
        #             select_dict[select[0]] = select[2]  
        ...
        #     selected_frames = list(select_dict.keys())[unlabelled_sample_num - int(self.cfg.active_train.select_ratio * unlabelled_sample_num):]
        ...
        #     saved_frames = selected_frames + [info['token'] for info in self.labelled_loader.dataset.data_infos]
        #     mmcv.dump(saved_frames, os.path.join(self.active_label_dir, 'saved_frames_epoch_{}.pkl'.format(cur_epoch)))
