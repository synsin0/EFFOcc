...
class BEVOCCFlowHead3D(BaseModule):
    def __init__(self,
                 in_dim=32,
                 out_dim=32,
                 use_mask=True,
        ...
        super(BEVOCCFlowHead3D, self).__init__()
        self.out_dim = 32
...
@HEADS.register_module(force=True)
class BEVOCCFlowHead2D(BaseModule):
    def __init__(self,
                 in_dim=256,
                 out_dim=256,
                 Dz=16,
        ...
        super(BEVOCCFlowHead2D, self).__init__()
        self.in_dim = in_dim
    ...
    def forward(self, img_feats):
        ...
        occ_feat = self.final_conv(img_feats).permute(0, 3, 2, 1)
        ...
        if self.use_predicter:
            ...
            flow_out = flow_out.view(bs, Dx, Dy, self.Dz, 2)

        return occ_pred, flow_out

    def loss(self,voxel_semantics, voxel_flow, mask_camera, occ_preds, flow):
        loss_ = dict()
        voxel_semantics=voxel_semantics.long()
        if self.use_mask:
