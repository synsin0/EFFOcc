...
class LSSViewTransformer(BaseModule):
    ...
    def __init__(
        ...
        if large_depth_net:
            self.depth_net = nn.Sequential(
                ...
                nn.BatchNorm2d(in_channels),
                nn.ReLU(True),
                nn.Conv2d(in_channels, self.D + self.out_channels, 1))
    ...
    def forward(self, input, depth_from_lidar=None, with_bevdepth_supervision=False):
        ...
        if self.with_depth_from_lidar:
            ...
            depth_from_lidar_feat = self.lidar_input_net(depth_from_lidar)
            
            x = torch.cat([x, depth_from_lidar_feat], dim=1)
        ...
        # with torch.no_grad():
        #     downsample_factor = depth_from_lidar.shape[-1] // depth.shape[-1]
        #     depth_from_lidar_viz = F.max_pool2d(depth_from_lidar, kernel_size=downsample_factor, stride=downsample_factor)
        #     depth_bins = torch.arange(self.grid_config['depth'][0], self.grid_config['depth'][1], self.grid_config['depth'][2]).to(depth.device)
        #     depth_pred_viz = torch.einsum('bnhw,n->bhw', depth, depth_bins)
        #     # print(torch.min(depth_from_lidar), torch.max(depth_from_lidar))
        #     # print(depth.shape, depth_from_lidar_viz.shape, depth_bins.shape, depth_pred_viz.shape)
        #     depth_from_lidar_viz_np = depth_from_lidar_viz.cpu().numpy()[0,0]
        #     depth_pred_viz_np = depth_pred_viz.cpu().numpy()[0]
        #     depth_viz = np.concatenate([depth_from_lidar_viz_np, depth_pred_viz_np])
        #     depth_viz = (depth_viz - np.min(depth_viz)) / (np.max(depth_viz) - np.min(depth_viz)) * 255
        #     cv2.imwrite('depth_viz.png', depth_viz)
        #     # exit(0)

        if self.with_depth_from_lidar and with_bevdepth_supervision:
            loss_depth = self.get_depth_loss(depth_from_lidar, depth)
            # print(depth.shape, depth_from_lidar.shape)
            # exit(0)
            return self.view_transform(input, depth, tran_feat), loss_depth
        return self.view_transform(input, depth, tran_feat)

    def get_depth_loss(self, depth_labels, depth_preds):
        downsample_factor = depth_labels.shape[-1] // depth_preds.shape[-1]
        depth_labels = self.get_downsampled_gt_depth(depth_labels, downsample_factor)
...
class DepthNet(nn.Module):
    ...
    def gen_grid(self, metas, B, N, D, H, W, hi, wi):
        ...
        points = points[..., :2, :] / (points[..., 2:3, :] + 1e-5)
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def get_depth_loss(self, depth_labels, depth_preds):    
        ...
        # print('depth correct sum: ', fg_mask.sum())

        # print('min max depth preds {} {}'.format(depth_preds.min(),depth_preds.max()))
        # print('min max depth labels {} {}'.format(depth_labels.min(),depth_labels.max()))
