...
class BEVDetOCCFlow(BEVDet):
    def __init__(self,
                 ...
                 loss_flow=None,
                 **kwargs):
        super(BEVDetOCCFlow, self).__init__(**kwargs)
    ...
    def loss_single(self,voxel_semantics, voxel_flow, mask_camera, occ_preds, flow):
        loss_ = dict()
    ...
    def simple_test(self,
        ...
        occ_feat = self.final_conv(img_feats[0]).permute(0, 4, 3, 2, 1)
        ...
        flow_out = self.flow_predicter(occ_feat)
    ...
    def forward_train(self,
        ...
        # img_feats: torch.Size([2, 32, 16, 200, 200])
        ...
        flow_pred = self.flow_predicter(occ_feat)
        ...
        loss_occ = self.loss_single(voxel_semantics, voxel_flow, final_mask, occ_pred, flow_pred)
        losses.update(loss_occ)
        return losses


@DETECTORS.register_module()
class BEVStereo4DOCCFlow(BEVStereo4D):
    ...
    def __init__(self,
                 ...
                 loss_flow=None,
                 **kwargs):
        super(BEVStereo4DOCCFlow, self).__init__(**kwargs)
        self.out_dim = out_dim
        out_channels = out_dim if use_predicter else num_classes
        self.final_conv = ConvModule(
                        self.img_view_transformer.out_channels,
    ...
    def forward_train(self,
        ...
        occ_feat = self.final_conv(img_feats[0]).permute(0, 4, 3, 2, 1) # bncdhw->bnwhdc
        if self.use_predicter:
            occ_pred = self.predicter(occ_feat)
        flow_pred = self.flow_predicter(occ_feat)
