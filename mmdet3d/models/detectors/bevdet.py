...
class BEVDepth(BEVDet):
    ...
    def extract_img_feat(self, img, img_metas, **kwargs):
        """Extract features of images."""
        img = self.prepare_inputs(img)
        imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda = img

        mlp_input = self.img_view_transformer.get_mlp_input(sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda)
        x, _ = self.image_encoder(imgs)

        inputs_curr = (x, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, bda, mlp_input)

        x, depth = self.img_view_transformer(inputs_curr)
        x = self.bev_encoder(x)
        return [x], depth
