...
from mmdet3d.models.query_strategies import build_strategy
import time
ACTIVE_LABELS = {}
NEW_ACTIVE_LABELS = {}
...
def select_active_labels(model,
    ...
    strategy = build_strategy(method=method, model=model, \
    ...
    if os.path.isfile(os.path.join(active_label_dir, 'selected_frames_epoch_{}.pkl'.format(cur_epoch))):
        ...
        output = strategy.query(leave_pbar, cur_epoch)
        # if rank==0:
        #     selected_frames, labeled_frames, unlabelled_frames = output
        #     import ipdb
        #     ipdb.set_trace()
