...
def create_calib_input_data_impl(calib_file: str,
    ...
    with h5py.File(calib_file, mode='w') as file:
        ...
        input_keys = [
            'points','gt_depth','ranks_bev', 'ranks_depth', 'ranks_feat', 'interval_starts',
            'interval_lengths'
        ]
        input_groups = []
...
def main():
    ...
    assert model.img_view_transformer.grid_size[0] == 200
    assert model.img_view_transformer.grid_size[1] == 200
    ...
    for i, data in enumerate(data_loader):
        ...
        points = data['points'][0].data[0][0].cuda()
        
        gt_depth = data['gt_depth'][0].cuda()
        metas = model.get_bev_pool_input(inputs)
        img = inputs[0].squeeze(0)
        with torch.no_grad():
            torch.onnx.export(
                model,
                (img.float().contiguous(), points.float().contiguous(), gt_depth.float().contiguous(), metas[1].int().contiguous(),
                ...
                input_names=[
                    'img', 'points', 'gt_depth', 'ranks_depth', 'ranks_feat', 'ranks_bev',
                    'interval_starts', 'interval_lengths'
                ],
                output_names=['output_occ'])
